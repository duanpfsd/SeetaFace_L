...
{
 public:
   FaceIdentifyRecoger() :dicFlag_(false)
  {
    memcpy(gallery_fea_, 0, sizeof(float)*kNumGallery*2048);
    memcpy(gallery_threhold_, 0, sizeof(float)*kNumGallery);
    memcpy(gallery_threhold_, "", sizeof(char)*kNumGallery);

    detector_ = nullptr;
    point_detector_ = nullptr;
    face_recognizer_ = nullptr;
  }
   ~FaceIdentifyRecoger(){}
 ...
 private:
  ...
  std::string name_pinyin_[kNumGallery];  // Pinyin of objects's names.
  ...
  seeta::FaceAlignment* point_detector_;
  seeta::FaceIdentification* face_recognizer_;
};
